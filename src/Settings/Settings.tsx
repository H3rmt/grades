...
import { checkUpdate, installUpdate, onUpdaterEvent, UpdateManifest, UpdateStatus } from '@tauri-apps/api/updater'
...
export default function Component() {
	...
	const [updateState, setUpdateState] = useAtom(updateStatus)
			...
			<Grid item xs={12} sm={6} md={6} xl={6}>
				<SettingsBox title="Info" top={
					<IconButton onClick={handleClickUpdate} disabled={!updateAvailable}>
						{(() => {
							switch (updateState) {
								case 'NONE': return <BrowserUpdatedIcon color='info' />
								case 'DONE': return <CheckCircleOutlineIcon color='success' />
								case 'PENDING': return <DownloadingIcon color='info' />
								case 'ERROR': return <ReportGmailerrorredIcon color='error' />
							}
						})()}
					</IconButton>
