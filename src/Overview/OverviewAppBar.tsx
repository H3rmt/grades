	...
	return <Stack spacing={2} direction="row" alignItems="center">
		...
		{(() => {
			...
			// }} title="New Grade"><AddIcon/></IconButton>
				...
				return <Button variant="contained" color="primary" component={LinkRef} to="/overview/newGrade"
					search={(old: NewGradeModalSearch) => {
						return {...old, confirmed: false}
					}} title="New Grade">New&nbsp;Grade</Button>
			else
				return <>
					<Button color="primary" variant="contained" component={LinkRef} to="/overview/newGrade" search={(old: NewGradeModalSearch) => {
						return {...old, confirmed: false}
